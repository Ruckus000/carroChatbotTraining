...
def test_api(text, url="http://localhost:8000"):
    ...
    try:
        response = requests.post(
            endpoint, json={"text": text}, headers={"Content-Type": "application/json"}
        )
        ...
        if response.status_code == 200:
            ...
            print("\nDetected Intent:")
            print(
                f"  {result['intent']['name']} (confidence: {result['intent']['confidence']:.4f})"
            )
...
if __name__ == "__main__":
    ...
    test_text = (
        sys.argv[1]
        if len(sys.argv) > 1
        else "My 2022 Honda Civic needs a tow from 123 Main Street"
    )
    test_api(test_text)
