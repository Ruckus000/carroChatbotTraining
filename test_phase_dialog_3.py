...
class MockNLUInferencer:
    ...
    def predict(self, text):
        ...
        elif "yes" in text_lower or "confirm" in text_lower or "that's all" in text_lower: # Simulate confirmation
...
class TestPhase3ResponseGeneration(unittest.TestCase):
    ...
    def test_response_for_request_slot(self):
        ...
        self.assertTrue(any(phrase in result["bot_response"].lower() for phrase in ["vehicle", "where", "pickup", "located", "tow truck"]))
    ...
    def test_response_for_completion(self):
        ...
        action = {"type": "RESPOND_COMPLETE", "details": state.entities, "intent": state.current_intent}
        response = self.manager.response_generator.generate_response(action, state)

        self.assertTrue(any(word in response.lower() for word in ["confirmed", "booked", "dispatched", "en route", "set", "help"]))
    ...
    def test_response_for_fallback(self):
        ...
        self.assertTrue(any(phrase in response.lower() for phrase in [
            "understand", "clarity", "rephrase", "didn't", "sorry", "got that", "more detail"
        ]))
        print(f"Response: {response}")
