...
class TestPhase2(unittest.TestCase):
    ...
    def test_intent_model_files_exist(self):
        ...
        self.assertTrue(
            os.path.exists('./trained_nlu_model/intent_model/pytorch_model.bin') or
            os.path.exists('./trained_nlu_model/intent_model/model.safetensors'),
    ...
    def test_entity_model_files_exist(self):
        ...
        # Check for either pytorch_model.bin or model.safetensors
        self.assertTrue(
            os.path.exists('./trained_nlu_model/entity_model/pytorch_model.bin') or
            os.path.exists('./trained_nlu_model/entity_model/model.safetensors'),
    ...
    def test_intent_mappings(self):
        ...
        try:
            ...
            for intent, id_val in intent2id.items():
                self.assertIsInstance(intent, str, f"Intent key is not a string: {intent}")
                self.assertIsInstance(id_val, int, f"Intent ID is not an integer: {id_val}")
    ...
    def test_entity_mappings(self):
        ...
        try:
            ...
            for tag, id_val in tag2id.items():
                ...
                self.assertIsInstance(id_val, int, f"Tag ID is not an integer: {id_val}")
            ...
            self.assertTrue(
                any(any(entity in entity_type for entity in common_entities) 
    ...
    def test_model_configs(self):
        ...
        try:
            ...
            self.assertIn('id2label', config, "Intent model config missing id2label mapping")
            self.assertIn('label2id', config, "Intent model config missing label2id mapping")
        ...
        try:
            ...
            self.assertIn('id2label', config, "Entity model config missing id2label mapping")
            self.assertIn('label2id', config, "Entity model config missing label2id mapping")
