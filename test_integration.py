...
def main():
    ...
    for i, test_case in enumerate(test_cases):
        ...
        try:
            ...
            # Display result
            print(
                f"  Intent: {result['intent']['name']} (confidence: {result['intent']['confidence']:.4f})"
            )
            ...
            intent_matched = any(
                intent_name.startswith(test_case["expected_intent_type"])
                for intent_type in [test_case["expected_intent_type"]]
            )

            if not intent_matched and not intent_name.startswith("fallback"):
                print(
                    f"  [FAIL] Intent '{intent_name}' does not match expected type '{test_case['expected_intent_type']}'"
                )
                all_passed = False
            else:
            ...
            if test_case["expected_entity_types"]:
                ...
                for expected_type in test_case["expected_entity_types"]:
                    ...
                    if not found:
                        print(
                            f"  [WARN] Expected entity type '{expected_type}' not found"
                        )
